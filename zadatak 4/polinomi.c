...
polinom* strvori_novog(int broj, int exponent);
...
int citaj_iz_datoteke(polinom* head1, polinom* head2,char*filename)
{
	...
	while (!feof(datoteka))
	{
		fgets(buffer, LINE, datoteka);
		p = buffer;
		if (sscanf(buffer, " %d %d %n", &temp_broj, &temp_exponenet,&n) == 2)
		{
			if (broj_linija == 1)
			{
...
polinom* strvori_novog(int broj, int exponent)
{
	polinom* temp=NULL;									//ovdje stvaram strukturu ali zasto nebi mogao stvorit samo position pokazivac na tu trukturu npr position p
...
int stvori_nakon(polinom* head, position pronadeni, position stvoreni)
{
	position hodajuci = NULL;							//dali da ja stvorim pokazivac na tu strukturu ili samo novu strukturu temp
